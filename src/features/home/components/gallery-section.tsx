      ...
      <div className="mx-auto max-w-[1281px] px-4 sm:px-6 lg:px-8 flex flex-col items-center gap-10">
        ...
        <div className="w-full max-w-[534px] h-[70px] relative flex flex-col items-center select-none">
          ...
          <div className="w-full h-[41px] relative mt-2 flex justify-center lg:justify-start">
            ...
            <div className="absolute w-[402px] h-[17px] inset-s-0 top-[24px] bg-rose-200/50 dark:bg-rose-950/40 rounded-e-[20px]" />

            {/* Rectangle 2 Accent Line */}
            <div className="absolute w-[157px] h-[2px] inset-s-0 top-[39px] bg-primary-600 dark:bg-rose-500" />

            {/* Text */}
            <h2 className="absolute inset-x-0 top-0 font-sarabun font-bold text-4xl leading-[36px] text-primary-600 dark:text-rose-200 z-10 whitespace-nowrap">
              {t("title")}
            </h2>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 lg:hidden w-full">
          {images.map((img) => (
            <div
              key={img.src}
              className="relative aspect-4/3 overflow-hidden rounded-xl shadow-sm group"
            >
              <Image
                src={img.src}
                alt={img.alt}
