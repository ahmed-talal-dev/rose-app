...
function buildFeatures(
  t: ReturnType<typeof useTranslations<"home.about">>,
): string[] {
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-12 lg:gap-20">
          ...
          <div className="flex flex-row items-center gap-2 shrink-0 select-none w-full max-w-[530px] h-[377px] justify-center lg:justify-start">
            ...
            <div className="relative w-[329px] h-[377px] shrink-0">
              {/* Rectangle 3 Border */}
              <div
                className="absolute w-[269px] h-[363px] left-0 top-0 border-4 border-rose-500 dark:border-rose-200 rotate-[3.09deg]"
          ...
          <div className="flex flex-col items-start gap-6 w-full lg:max-w-[627px]">
            {/* About label */}
            <span className="text-primary-600 dark:text-rose-200 font-sarabun font-bold text-base leading-[21px] tracking-[0.25em] uppercase">
              {t("eyebrow")}
            </span>

            {/* Title: Delivering the Finest Gift Boxes for Your Special Moments */}
            <h2 className="font-sarabun font-bold text-[30px] leading-tight text-primary-600 dark:text-rose-200">
              {t("titlePart1")}
              <span className="text-rose-500">{t("titleHighlight1")}</span>
              {t("titlePart2")}
              <span className="text-rose-500">{t("titleHighlight2")}</span>
              {t("titlePart3")}
            </h2>

            {/* Description */}
            <p className="text-muted-foreground font-sarabun font-normal text-base leading-relaxed dark:text-zinc-300/80">
              {t("description")}
            </p>
            ...
            <Link
              href="/products"
              className="flex flex-row justify-center items-center px-5 py-2.5 gap-2.5 bg-primary-600 hover:bg-rose-200/90 dark:bg-rose-200 dark:hover:bg-primary-600 rounded-[10px] text-white dark:text-zinc-800 transition-all shrink-0 shadow-sm font-medium group"
            >
              <span className="font-sarabun text-base leading-none">
                {t("cta")}
              </span>
              <ArrowRight className="h-4 w-4 text-white dark:text-zinc-800 transition-transform group-hover:translate-x-1" />
            </Link>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 w-full pt-4">
              {features.map((feature) => (
                <div key={feature} className="flex items-center gap-3">
                  <Check
                    className="h-5 w-5 text-rose-500 dark:text-rose-200 shrink-0"
                    strokeWidth={3}
                  />
                  <span className="text-sm font-normal text-foreground font-sarabun dark:text-zinc-300">
                    {feature}
                  </span>
                </div>
              ))}
