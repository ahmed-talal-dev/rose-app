        ...
        return (
            <div className="flex w-full flex-col items-center gap-4 text-center">
                <div className="flex size-14 items-center justify-center rounded-full bg-primary-50">
                    <svg className="size-7 text-primary-700" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                    </svg>
                </div>
                <div className="flex flex-col gap-1">
                    <p className="text-sm font-semibold text-zinc-800 font-inter">{t("checkEmail")}</p>
                    <p className="text-xs text-zinc-500 font-inter">{t("checkEmailSubtitle")}</p>
                </div>
                <div className="w-full border-t border-zinc-200 mt-2" />
                <p className="text-xs lg:text-sm font-medium text-zinc-800 font-sarabun">
                    {t("noAccount")}{" "}
                    <Link href="/register" className="font-semibold text-primary-700 hover:underline font-sarabun">
                        {t("createAccount")}
                    </Link>
                </p>
            </div>
            ...
            <div className="flex flex-col gap-2 lg:gap-2.5">
                <div className="flex flex-col gap-1">
                    ...
                    <Input
                        ...
                        {...register("email")}
                        className={`h-10 lg:h-12.25 rounded-[10px] border-zinc-300 px-4 text-sm font-inter text-zinc-800 placeholder:text-zinc-400 ${errors.email ? "border-red-500" : ""}`}
                    />
                    {errors.email && (
