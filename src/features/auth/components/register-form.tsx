        ...
        onError: (err: ApiError) => {
          if (err.errors?.length) {
            err.errors.forEach(({ path, message }) => {
              registerForm.setError(path as keyof RegisterSchema, {
                type: "server",
                message,
              });
            });
      ...
      >
        <div className="flex flex-col gap-2 lg:gap-3.5">
          ...
          <div className="flex w-full gap-3 lg:gap-5">
            {(["firstName", "lastName"] as const).map((field) => (
              <div key={field} className="flex flex-1 flex-col gap-1">
                ...
                />
                <FieldError
                  message={registerForm.formState.errors[field]?.message}
                />
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-col gap-1">
            ...
            />
            <FieldError
              message={registerForm.formState.errors.phone?.message}
            />
          </div>
