...
const buildLoginSchema = (
  t: ReturnType<typeof useTranslationsType<"auth.login.form">>,
) =>
  z.object({
...
export function LoginPopover() {
  ...
  return (
    <div className="w-[393px] h-[370px] bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-[0px_10px_30px_rgba(0,0,0,0.08)] flex flex-col overflow-hidden font-sarabun text-start select-none">
      {/* Tabs */}
      <div className="flex flex-row items-start w-[393px] h-11 shrink-0">
        {/* Login Tab */}
        <button
          type="button"
          onClick={() => setActiveTab("login")}
          className={`flex flex-row justify-center items-center px-4 py-3 gap-2.5 w-[196.5px] h-11 transition-colors cursor-pointer border-none outline-none ${
            activeTab === "login"
        ...
        >
          ...
          <div className="flex flex-col gap-4 w-full">
            ...
            <div className="flex flex-col items-start gap-1.5 w-full h-[72px] bg-white dark:bg-zinc-900 shrink-0">
              <label className="h-[17px] text-sm font-medium text-zinc-800 dark:text-zinc-300 font-inter">
                {t("emailLabel")}
              </label>
              <input
                type="text"
                ...
                {...register("username")}
                className={`w-full h-12 bg-white dark:bg-zinc-800 border border-zinc-300 dark:border-zinc-700 rounded-xl px-4 py-4 text-sm text-zinc-800 dark:text-zinc-100 font-inter placeholder-zinc-400 dark:placeholder-zinc-500 outline-none focus:border-primary-600 dark:focus:border-rose-500 transition-colors ${
            ...
            <div className="flex flex-col items-start gap-1.5 w-full h-[72px] bg-white dark:bg-zinc-900 shrink-0">
              ...
              <div className="relative w-full">
                ...
                <button
                  ...
                  className="absolute inset-y-0 inset-e-4 flex items-center justify-center text-zinc-400 hover:text-zinc-600 dark:text-zinc-500 dark:hover:text-zinc-300 transition-colors cursor-pointer"
                >
                  {showPassword ? (
          ...
          <div className="flex flex-col w-full gap-4 shrink-0">
            {/* Checkbox (light) */}
            <label className="flex flex-row items-center pt-5 gap-2.5 w-full h-10 cursor-pointer select-none">
              <input
                type="checkbox"
            ...
            <button
              type="submit"
              disabled={isPending}
              className="flex justify-center items-center w-full h-10 bg-primary-600 hover:bg-primary-700 transition-colors rounded-xl text-white font-sarabun font-medium text-base disabled:opacity-75 cursor-pointer"
            >
              {isPending ? (
