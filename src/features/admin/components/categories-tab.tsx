...
type CategoryWithCount = Category & { productsCount?: number };
...
export function CategoriesTab({
    ...
    // Derived data —
    const categories: CategoryWithCount[] = categoriesData?.data ?? [];
    const totalPages = categoriesData?.metadata?.totalPages ?? 1;
                                        ...
                                        >
                                            ...
                                            <td className="px-4 md:px-6 py-3 text-sm text-right relative">
                                                ...
                                                <div className="flex md:hidden justify-end">
                                                    <button
                                                        type="button"
                                                        onClick={() => setActiveRowMenuId(activeRowMenuId === cat.id ? null : cat.id)}
                                                    ...
                                                    {activeRowMenuId === cat.id && (
                                                        <div className="absolute right-4 top-10 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg shadow-lg p-1 w-24 z-20 flex flex-col gap-0.5">
                                                            <button
                                                                type="button"
                                                                onClick={() => { setActiveRowMenuId(null); onOpenEdit(cat); }}
                                                            ...
                                                            <button
                                                                type="button"
                                                                onClick={() => { setActiveRowMenuId(null); onDelete(cat); }}
